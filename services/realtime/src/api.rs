use crate::auth::jwt::{authorization_from_token, UserUuid};
...
pub type RemoteRealtimeServerAddr = Addr<RemoteRealtimeServerActor>;
...
pub async fn establish_ws_connection_v1(
  ...
  payload: Payload,
  state: Data<AppState>,
  server: Data<RemoteRealtimeServerAddr>,
  web::Query(query_params): web::Query<HashMap<String, String>>,
) -> Result<HttpResponse> {
  println!("🚀new websocket connect: request={:?}", request);
